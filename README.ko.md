...
- **텔레그램**: picker에서 `^T`(켜지면 `telegram ✓` 배지) — Claude Code의 텔레그램 채널을 붙인 채로 세션을 시작한다. `^T`는 먼저 플러그인이 켜져 있고 봇 토큰이 있는지 확인하고, 없으면 뭐가 빠졌는지 알려준다(`/plugin`, `/telegram:configure`) — Claude Code는 그냥 시작하고 채널만 조용히 안 붙기 때문. 플러그인 쪽 제약(0.0.7): 봇은 한 번에 한 Claude Code 세션만 잡을 수 있고, 플러그인이 켜진 새 Claude Code 세션이 열리면 그쪽이 가져간다
...
- **tmux 밖** — picker가 뜬다. 위쪽엔 실행 중인 세션(`Enter`로 그 세션에 attach — picker가 만들지 않고 합류하는 유일한 경우), 아래엔 새 Claude Code 세션을 시작할 디렉토리. 무엇을 고르든 pier가 attach까지 한다. 마지막 세션이 끝나면 `[exited]` 흔적 없이 원래 프롬프트로 돌아온다.
...
| 새 세션 생성 | `+ new session` 클릭(사이드바 포커스에서 `n`, 아무 데서나 `prefix+N`, tmux 밖에서는 그냥 `pier`). 타이핑으로 필터 — 실행 중인 세션과 디렉토리(`~/dev/*` + 과거 CC 사용 경로) 양쪽에 걸린다. `Enter` 생성, `^S`는 Claude Code 대신 터미널 세션(일반 셸)으로 생성, `^T`는 텔레그램 채널 부착, `Tab`으로 제안된 이름 수정. 없는 경로를 입력하면 `mkdir & create`로 디렉토리까지 만들어 진행. 실행 중인 세션은 맨 위에, 현재 세션이 `▌`로 가장 위에 오고 커서도 거기서 시작 — 실행 중 세션 행의 `Enter`는 그 디렉토리에 Claude Code 세션 하나 더(`-2`, `-3`, … 접미사), `^S`는 그 디렉토리에 터미널. tmux 안에서 picker는 세션을 전환하지 않는다(전환은 사이드바 클릭). tmux 밖에서는 실행 중 세션 행의 `Enter`가 attach. 모든 키는 `▸` 커서 행에 작용하며, popup이 떠 있는 동안 그대로 누른다(prefix 불필요) |
