...
- **새 세션 생성**: `+ new session` 클릭(또는 `n`) — 중앙 popup에서 디렉토리를 고르면 그 이름의 Claude Code 세션이 그 경로에서 시작된다
...
| 새 세션 생성 | `+ new session` 클릭(또는 사이드바 포커스에서 `n`). 타이핑으로 디렉토리 필터(`~/dev/*` + 과거 CC 사용 경로), `Enter` 생성, `Tab`으로 제안된 이름 수정. 이미 열린 경로를 고르면 생성 대신 점프 |
